...
use crate::core::compiler::{BuildConfig, CompileKind, CompileMode, DefaultExecutor, Executor};
...
pub fn package(ws: &Workspace<'_>, opts: &PackageOpts<'_>) -> CargoResult<Option<Vec<FileLock>>> {
    ...
    let requested_kinds = CompileKind::from_requested_targets(opts.config, &*opts.targets)?;

    if ws.root().join("Cargo.lock").exists() {
        // Make sure the Cargo.lock is up-to-date and valid.
        let _ = ops::resolve_ws(ws, &*requested_kinds)?;
...
fn build_lock(
    ...
    let (pkg_set, mut new_resolve) = ops::resolve_ws(&tmp_ws, requested_kinds)?;
